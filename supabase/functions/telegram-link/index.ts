...
Deno.serve(async (req: Request) => {
  ...
  // Auth check — extract user from JWT in Authorization header
  ...
  // ── GET /status — check if Telegram is already linked ──
  ...
  if (req.method === "POST") {
    ...
    if (insertError) {
      ...
      return new Response(JSON.stringify({ error: "Failed to generate link token" }), {
        status: 500, headers: { "Content-Type": "application/json", ...corsHeaders },
      });
    }

...
function App() {
  ...
  useEffect(() => {
    ...
    if (shareId) {
      ...
      const fetchSharedFile = async () => {
        try {
          const response = await fetch(`${SERVER_URL}/api/shared-file/${shareId}`, {
            method: 'GET',
            headers: {
          ...
          toast.error(`Error processing shared image: ${error.message}`);
        }
      };
  ...
  const handleExpenseImage = (file: File) => {
    console.log('📷 Processing expense image:', file.name, file.type, file.size);
...
export default App;_api_key")
  ...
  useEffect(() => {
    ...
    if (shareId) {
      ...
      const fetchSharedFile = async () => {
        try {
          const response = await fetch(`/api/shared-file/${shareId}`);
          const data = await response.json();
